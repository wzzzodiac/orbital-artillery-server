...
function isVoidDeath(player){return player?.motion?.type==='knockbackVoid'||Number(player?.spawn?.y)>5000||Number(player?.motion?.toY)>5000;}
...
function applyCorrectedHp(room,q,damageByPlayer,now,{consumeShield=true}={}){
  ...
  for(const player of room.players){
    ...
    if(baseHp==null||wasAlive===false||raw<=0)continue;if(isVoidDeath(player))continue;
    const hadShield=Boolean(snap.shield[player.id]),damage=hadShield?Math.max(1,Math.ceil(raw*SHIELD_FACTOR)):raw,hp=Math.max(0,baseHp-damage);
    if(player.hp!==hp||player.lastDamage?.amount!==damage)changed=true;
    player.hp=hp;player.alive=hp>0;player.lastDamage={amount:damage,at:now,sourcePlayerId:q.ownerPlayerId};if(hp<=0)player.motion=null;
    if(hadShield)player.shield=consumeShield?null:copyShield(snap.shield[player.id]);
  }
...
function tripleDamage(q){const out={};for(const shot of q.volley??[]){if(shot?.impactReason!=='player'||!shot.hitPlayerId)continue;out[shot.hitPlayerId]=(out[shot.hitPlayerId]??0)+TRIPLE_DAMAGE_PER_HIT;}return out;}
...
function correctResolvedBallistic(room,q,now){
  ...
  if(q.weaponType==='basic'&&q.resolutionApplied&&!q.v09DamageAdjusted){q.v09DamageAdjusted=true;return applyCorrectedHp(room,q,radialDamage(q,BASIC_DAMAGE,BASIC_RADIUS,{directMax:true}),now);}
  if(q.weaponType==='heavy'&&q.heavyResolutionApplied&&!q.v09DamageAdjusted){q.v09DamageAdjusted=true;return applyCorrectedHp(room,q,radialDamage(q,HEAVY_DAMAGE,HEAVY_RADIUS,{directMax:true}),now);}
  if(q.weaponType==='triple'&&q.specialResolutionApplied&&!q.v09DamageAdjusted){q.v09DamageAdjusted=true;return applyCorrectedHp(room,q,tripleDamage(q),now);}
  if(q.weaponType==='cluster'&&q.specialResolutionApplied&&!q.v09DamageAdjusted){q.v09DamageAdjusted=true;return applyCorrectedHp(room,q,clusterDamage(q),now);}
  return false;
}
function newlyAppliedAirShells(q,before){const appliedBefore=new Set(before??[]);return(q?.airStrikeShells??[]).filter(shell=>shell.applied&&!appliedBefore.has(shell.index));}
function correctAirStrike(room,q,beforeHp,beforeApplied,now){
  ...
  q.v09ShieldHitPlayerIds=[...shieldHits];room.match.pendingResult=resultFor(room);return true;
}
export function fireProjectile6DV09(id){
  const room=findRoomBySocket(id),player=room?.players.find(p=>p.id===id),beforeHp=player?.hp??null,result=baseFire(id);if(!result.ok)return result;const q=result.room.match?.projectile;if(q)snapshotAttack(result.room,q);
  if(!q&&player?.lastUtility?.type==='heal'&&beforeHp!=null){const desired=Math.min(MAX_HP,beforeHp+HEAL_AMOUNT),healed=desired-beforeHp;player.hp=desired;player.lastUtility={...player.lastUtility,label:`HEAL +${healed} HP`,amount:healed};result.healed=healed;}return result;
}
export function advanceTurnIfDue6DV09(code,now=Date.now()){
  const room=getRoom(code),q=room?.match?.projectile;if(q)snapshotAttack(room,q);const beforeHp=q?Object.fromEntries(room.players.map(p=>[p.id,p.hp??MAX_HP])):{},beforeApplied=q?.weaponType==='airstrike'?(q.airStrikeShells??[]).filter(s=>s.applied).map(s=>s.index):[];
