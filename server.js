...
const httpServer = createServer((req, res) => {
  if (req.url === '/health') { res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' }); res.end(JSON.stringify({ ok: true, service: 'orbital-artillery-server', phase: '7A.1-hotfix', rooms: roomStore.size, sockets: io?.engine?.clientsCount ?? 0 })); return; }
...
function publicState(room) { ensureAfkVoteState(room); return publicRoomState7AHotfix(room); }
...
setInterval(() => {
  ...
  for (const room of roomStore.values()) {
    ...
    if (room.status === 'countdown' && now >= room.match?.startAt) changed = activateRoom(room.code, now);
      ...
      for (let catchUp = 0; catchUp < 8; catchUp += 1) { const advanced = advanceTurnIfDue7AHotfix(room.code, now); if (!advanced) break; changed = advanced; }
    }
...
io.on('connection', socket => {
  ...
  socket.on('set_terrain', (payload, reply = () => {}) => { if (!allowRoomAction()) return reply({ ok: false, error: 'room_action_rate_limited' }); replyMutation(setTerrain7AHotfix(socket.id, String(payload?.terrain ?? '').toLowerCase()), reply); });
  ...
  socket.on('rematch_game', (payload, reply = () => {}) => { if (!allowRoomAction()) return reply({ ok: false, error: 'room_action_rate_limited' }); replyMutation(rematchRoom7AHotfix(socket.id, { randomMap: Boolean(payload?.randomMap) }), reply); });
  socket.on('move_player', (payload, reply = () => {}) => replyTurnAction(moveActivePlayer7AHotfix(socket.id, payload?.direction), reply));
  socket.on('jump_player', (payload, reply = () => {}) => replyTurnAction(jumpActivePlayer7AHotfix(socket.id, payload?.direction), reply));
  socket.on('set_aim', (payload, reply = () => {}) => replyTurnAction(setAim7AHotfix(socket.id, payload?.angle, payload?.power), reply));
  socket.on('select_item', (payload, reply = () => {}) => replyTurnAction(selectItem7AHotfix(socket.id, payload?.slot), reply));
  socket.on('fire_projectile', (_payload, reply = () => {}) => replyTurnAction(fireProjectile7AHotfix(socket.id), reply));
  socket.on('toggle_afk_skip_vote', (_payload, reply = () => {}) => {
    const result = toggleAfkSkipVote(socket.id);
    if (!result.ok) return reply(result);
    let room = result.room;
    if (result.skipped) room = advanceTurnIfDue7AHotfix(room.code, Date.now()) ?? room;
  ...
  idleTimer.unref();
  socket.on('disconnect', reason => { clearInterval(idleTimer); const removal = disconnectPlayer7AHotfix(socket.id); if (removal?.room) emitRoomState(removal.room); console.info(`socket disconnected: ${socket.id} (${reason})`); });
});
