...
import {
  advanceTurnIfDue7AVisual,
  disconnectPlayer7AVisual,
  fireProjectile7AVisual,
  jumpActivePlayer7AVisual,
  moveActivePlayer7AVisual,
  publicRoomState7AVisual,
  rematchRoom7AVisual,
  selectItem7AVisual,
  setAim7AVisual,
  setTerrain7AVisual
...
setInterval(() => {
  ...
  for (const room of roomStore.values()) {
    ...
    if (room.status === 'countdown' && now >= room.match?.startAt) changed = activateRoom(room.code, now);
      ...
      for (let catchUp = 0; catchUp < 8; catchUp += 1) { const advanced = advanceTurnIfDue7AVisual(room.code, now); if (!advanced) break; changed = advanced; }
    }
...
io.on('connection', socket => {
  ...
  socket.on('set_terrain', (payload, reply = () => {}) => { if (!allowRoomAction()) return reply({ ok: false, error: 'room_action_rate_limited' }); replyMutation(setTerrain7AVisual(socket.id, String(payload?.terrain ?? '').toLowerCase()), reply); });
  ...
  socket.on('rematch_game', (payload, reply = () => {}) => { if (!allowRoomAction()) return reply({ ok: false, error: 'room_action_rate_limited' }); replyMutation(rematchRoom7AVisual(socket.id, { randomMap: Boolean(payload?.randomMap) }), reply); });
  socket.on('move_player', (payload, reply = () => {}) => replyTurnAction(moveActivePlayer7AVisual(socket.id, payload?.direction), reply));
  socket.on('jump_player', (payload, reply = () => {}) => replyTurnAction(jumpActivePlayer7AVisual(socket.id, payload?.direction), reply));
  socket.on('set_aim', (payload, reply = () => {}) => replyTurnAction(setAim7AVisual(socket.id, payload?.angle, payload?.power), reply));
  socket.on('select_item', (payload, reply = () => {}) => replyTurnAction(selectItem7AVisual(socket.id, payload?.slot), reply));
  socket.on('fire_projectile', (_payload, reply = () => {}) => replyTurnAction(fireProjectile7AVisual(socket.id), reply));
  socket.on('toggle_afk_skip_vote', (_payload, reply = () => {}) => {
    const result = toggleAfkSkipVote(socket.id);
    if (!result.ok) return reply(result);
    let room = result.room;
    if (result.skipped) room = advanceTurnIfDue7AVisual(room.code, Date.now()) ?? room;
