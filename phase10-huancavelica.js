...
function installHuancavelicaArena(room){
  ...
  room.arena.phase10Theme=HUANCAVELICA_ID;room.arena.terrainName=HUANCAVELICA_NAME;room.arena.collisionModel='bitmap-alpha-mask-v2';room.arena.platforms=publicPlatforms();room.arena.voidFloor=true;room.arena.legacyCollisionBase=COLLISION_BASE;room.arena.bitmapTerrain={width:HUANCAVELICA_IMAGE_WIDTH,height:HUANCAVELICA_IMAGE_HEIGHT,worldWidth:WORLD_WIDTH,worldHeight:WORLD_HEIGHT,alphaThreshold:HUANCAVELICA_ALPHA_THRESHOLD,maskSource:'terrain-alpha',movementAuthority:'bitmap-mask'};
...
function decoratePublicState(room,state){
  ...
  if(isHuancavelica(room)){installHuancavelicaArena(room);state=basePublicRoomState9(room);state.terrainPresets=presets;state.terrainPreset=HUANCAVELICA_ID;state.arena={...(state.arena??{}),terrainPreset:HUANCAVELICA_ID,terrainName:HUANCAVELICA_NAME,phase10Theme:HUANCAVELICA_ID,collisionModel:'bitmap-alpha-mask-v2',platforms:publicPlatforms(),voidFloor:true,legacyCollisionBase:COLLISION_BASE,bitmapTerrain:{width:HUANCAVELICA_IMAGE_WIDTH,height:HUANCAVELICA_IMAGE_HEIGHT,worldWidth:WORLD_WIDTH,worldHeight:WORLD_HEIGHT,alphaThreshold:HUANCAVELICA_ALPHA_THRESHOLD,maskSource:'terrain-alpha',movementAuthority:'bitmap-mask'}};state.players=(state.players??[]).map(p=>({...p,phase10PlatformId:room.players.find(source=>source.id===p.id)?.phase10PlatformId??null}));state.phase10Map={id:HUANCAVELICA_ID,name:HUANCAVELICA_NAME,visualTheme:'authored-wild-ones-bitmap-layers',collisionModel:'bitmap-alpha-mask-v2',movementAuthority:'bitmap-mask',platformGraphRole:'spawn-and-navigation-metadata-only',platformCount:HUANCAVELICA_PLATFORMS.length,freeMovement:true,normalJumpDistance:NORMAL_JUMP_DISTANCE,adaptiveMaxDistance:MAX_LINK_JUMP,adaptiveMaxApex:MAX_ADAPTIVE_APEX,assetWidth:HUANCAVELICA_IMAGE_WIDTH,assetHeight:HUANCAVELICA_IMAGE_HEIGHT,maskSource:'terrain-alpha',productionReady:true};}
  return state;
}
