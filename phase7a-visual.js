...
const VEHICLE_HIT_RADIUS=78;
const HIT_SCAN_DT=.01;
const BASIC_MIN_VISIBLE_MS=4000;
const BOX_MIN_VISIBLE_MS=4667;
...
function stretchBallistic(v,minVisibleMs){
  if(!v||!Number.isFinite(v.startedAt)||!Number.isFinite(v.impactAt)||v.durationMs>=minVisibleMs)return;
  const oldDuration=Math.max(1,v.durationMs),scale=minVisibleMs/oldDuration;
  v.vx=Number(v.vx||0)/scale;v.vy=Number(v.vy||0)/scale;v.gravity=Number(v.gravity||0)/(scale*scale);v.windAccel=Number(v.windAccel||0)/(scale*scale);v.durationMs=minVisibleMs;v.impactAt=v.startedAt+minVisibleMs;
}
...
function expandProjectileHitbox(room,q){
  ...
  if(q.weaponType==='triple'&&Array.isArray(q.volley)){
    for(const v of q.volley){const hit=expandOneProjectileHit(room,v,q.ownerPlayerId);if(hit){stretchBallistic(v,BOX_MIN_VISIBLE_MS);changed=true;}}
  ...
  stretchBallistic(q,q.weaponType==='basic'?BASIC_MIN_VISIBLE_MS:BOX_MIN_VISIBLE_MS);
...
function delayAuthoritativeProjectile(room,q,delay=PROJECTILE_LAUNCH_HOLD_MS){
  ...
  q.startedAt=shiftTime(q.startedAt,delay);q.impactAt=shiftTime(q.impactAt,delay);q.resolveAt=shiftTime(q.resolveAt,delay);q.specialResolveAt=shiftTime(q.specialResolveAt,delay);q.targetLockedAt=shiftTime(q.targetLockedAt,delay);q.warningUntil=shiftTime(q.warningUntil,delay);q.beamAt=shiftTime(q.beamAt,delay);q.beamUntil=shiftTime(q.beamUntil,delay);
...
function paceAirStrike(room,q){
  ...
  q.airStrikeShells=(q.airStrikeShells??[]).map((shell,index)=>{const visualStartAt=warningEnd+index*AIR_STRIKE_STAGGER_MS;const impactAt=visualStartAt+AIR_STRIKE_FALL_MS;return{...shell,startY:Math.max(80,shell.y-AIR_STRIKE_START_RISE),visualStartAt,impactAt};});
...
export function publicRoomState7AVisual(room){const state=basePublic(room);state.visualHardening={projectileTrail:'full-flight',projectileAuthority:'single-authoritative-timeline',projectileLaunchHoldMs:PROJECTILE_LAUNCH_HOLD_MS,vehicleHitRadius:VEHICLE_HIT_RADIUS,airStrikeFallMs:AIR_STRIKE_FALL_MS,airStrikeStaggerMs:AIR_STRIKE_STAGGER_MS,projectilePlaybackSpeedMultiplier:1.5,nukeTerrainMode:'diagonal-scar',nukeScarDepth:NUKE_SCAR_DEPTH,nukeDamage:NUKE_DAMAGE};return state;}
export function fireProjectile7AVisual(id){const result=baseFire(id);if(!result.ok)return result;const q=result.room.match?.projectile;if(q?.weaponType==='airstrike')paceAirStrike(result.room,q);else{expandProjectileHitbox(result.room,q);delayAuthoritativeProjectile(result.room,q);}return result;}
export function advanceTurnIfDue7AVisual(code,now=Date.now()){const room=getRoom(code),q=room?.match?.projectile;if(room&&q?.weaponType==='nuke'&&!q.nukeApplied&&now>=q.beamAt){const before=phase7a1TestHooks.snapshot(room);if(applySafeNuke(room,q,now))phase7a1TestHooks.observe(room,before,{sourceId:q.ownerPlayerId,weaponType:'nuke'});}return baseAdvance(code,now);}
...
export const phase7aVisualTestHooks=Object.freeze({paceAirStrike,applySafeNuke,distanceToSegment,delayAuthoritativeProjectile,expandProjectileHitbox,expandOneProjectileHit,stretchBallistic});
